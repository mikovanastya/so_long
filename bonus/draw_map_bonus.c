...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/12 17:22:14 by rtwitch           #+#    #+#             */
/*   Updated: 2022/03/15 16:20:17 by rtwitch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_image	ft_new_sprite(void *mlx, char *path)
{
	...
	img.ptr = mlx_xpm_file_to_image(mlx, path, &img.size.x, &img.size.y);//создает новый образ
	img.pixels = mlx_get_data_addr(img.ptr, &img.bit_per_pixel, &img.line_size,
			&img.endian);//возвращает информацию о созданном изображении, позволяя пользователю изменить его позже.
	return (img);
}
...
void	draw_sprite_ground(t_prog prog, int i, int j)
{
	...
	mlx_put_image_to_window(prog.mlx, prog.window.ptr,
		prog.sprite.ptr, prog.sprite.pos.x, prog.sprite.pos.y);//можно рисовать внутри изображени и в любое время вывести на экран
...
void	steps(t_prog *prog)
{
	...
	mlx_string_put(prog->mlx, prog->window.ptr, 25, 15, 0x701370, str);//рисует определенный пиксель в окне
	free(str);
}
